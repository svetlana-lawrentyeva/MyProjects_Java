  ...
  public PaintPanel(ColorPanel cp, LinePanel lp, FigurePanel fp)
  {
   ...
   private void formComponentResized(java.awt.event.ComponentEvent evt)
        {                                      
            ...
            if ((w > 1) && (h > 1))
            {
                Image temp=new BufferedImage(w,h,BufferedImage.TYPE_3BYTE_BGR);
                if(bufImg!=null)
                {
                    temp=bufImg;
                }
   ...
   public void setImage(BufferedImage im)
   {
     ...
     tempImage = new BufferedImage(bufImg.getWidth(), bufImg.getHeight(), BufferedImage.TYPE_3BYTE_BGR);
     grTemp = (Graphics2D) tempImage.getGraphics();
